<script setup>
    ...
    const fetchDataFirst = async(ws_computerName = '', sched_year = 0, location_name = '', pdept_batch = 0, ws_userName = '', wsq_status = 0, wsq_id = 0) => {
        ...
        var data = {
            ...
            wsq_id: (wsq_id == null) ? 0 : wsq_id
        };

        const params = new URLSearchParams(data);

        let result = await axios.get(`/getListOfUserWorkstationWithRange?${params}`);
        ...
        if(result.data.status == Constants.STATUS_CODE_SUCCESS){
            ...
            logger.error("Error in Fetching the first batch list of the workstation", logger.createErrorContext("fetch-first-workstation-list", result.data.message));
    ...
    const fetchNextSet = async() => {
        if(!hasReachEnd.value){
            ...
            if(result.data.status == Constants.STATUS_CODE_SUCCESS){
                ...
                logger.error("Error in Fetching the list of workstation", logger.createErrorContext("fetch-workstation-list", result.data.message));
                utilStore.setSnackBar({
                    showModal: true,
                    modalDescription: result.data.message
                });
    ...
    const search = async() => {
        await fetchDataFirst(searchWorkstation.ws_computerName, searchWorkstation.sched_year, searchWorkstation.location_name, searchWorkstation.pdept_batch, searchWorkstation.ws_userName, searchWorkstation.wsq_status, searchWorkstation.wsq_id);
        ...
        fetchNextSet_pWsqId = (searchWorkstation.wsq_id == null) ? 0 : searchWorkstation.wsq_id;
                ...
                >
                    ...
                    >
                        ...
                        <v-text-field
                            ...
                            maxlength="255"
                            v-model.trim = searchWorkstation.wsq_id
                        >
                        </v-text-field>
                    </v-col>
                    <v-col
